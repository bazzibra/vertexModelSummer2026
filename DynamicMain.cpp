...
const double GAMMA0 = 10;		//!< GAMMA at the very beginning of the simulation (the const GAMMA the tissue was equilibrated with) 
...
double LYnew = 0;
//double TAU = 1;
double TAU_STRESS = 5;						//seems to be the time constant for stress relaxation
double A0SKEW = 1;
double LX0 = 0;
double LY0 = 0;
double T1PERCENT = 0;
...
double ExternalStressRatio = 0.85;
double equilibriumtolerance = 1e-9;
...
{	
	...
	TISSUE_EXT_STRESS_Y = -ExternalStressRatio*EpsilonMax*Y*sqrt(tissue.cells.size()) / LX;
	//cout << "Calculated Y-Stress Override:" << TISSUE_EXT_STRESS_Y << endl;
	...
	int necessaryEquilibratedTimeSteps = 20;
	int numberofEquilibratedTimeSteps = 0;
	bool strictlyEquilibrated = false;
	...
	while (CURRENTTIME < TOTALTIME) //run until totaltime
	{
		...
		//Check if the system has been equilibrated for a certain number of timesteps, if it has then it will stop the simulation and write out the data
		if(strictlyEquilibrated){numberofEquilibratedTimeSteps++;}
